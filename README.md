...
- 浏览器里看到的就是你 LAN 上 DLNA 客户端能看到的同一份目录，外加：**缩略图、横向滚动、音乐播放器、照片大图、自动同步**。

## 特性

- **概览页**：媒体计数、磁盘占用、扫描状态、已连接的 DLNA 客户端、自动同步状态、最近添加的视频 / 照片 / 音频。
- **媒体库浏览**：`全部 / 视频 / 照片 / 音乐` 四个 tab，按媒体目录下的"日期文件夹"分组，无限滚动分页（每页 60 项 + IntersectionObserver 触底加载）。
- **音乐播放器**：网易云风格，黑胶旋转封面（基于歌名 hash 自动配色）、自定义进度条、大号播放按钮。
- **照片大图**：全屏 lightbox，左右切换 / Esc 关 / 点图放大 / 下载原图。
- **视频播放**：浏览器内 HTML5 `<video>`（HEVC 看你浏览器支不支持），同时给出原始 DLNA 直链方便复制到投影仪等局域网客户端。
- **设置页**：直接 GUI 增删媒体目录、查看 minidlna 日志、一键重启服务、批量预生成视频缩略图。
- **自动同步**：30 秒轮询每个 media_dir 的顶层条目（不递归，避免 launchd 下的 TCC / fd 坑），检测到变化防抖 30 秒后给 minidlna `SIGHUP` 触发增量重扫。
- **缩略图**：ffmpeg 按需生成 320px JPG，磁盘缓存 (`~/.cache/minidlna-dashboard/thumbs/`)，并发限制 3 个 ffmpeg 进程避免外置盘抖动。

## 截图

| 媒体库浏览 | 视频文件夹（缩略图墙） |
| --- | --- |
| ![browse](docs/screenshot-browse.png) | ![videos](docs/screenshot-videos.png) |
...
- **概览**：[`http://<IP>:8201/`](http://localhost:8201/)
- **媒体库**：`/browse?kind=all`（或 `video` / `image` / `audio`）
- **设置**：`/settings`
